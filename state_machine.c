...
October 27, 2018
beepy blinky toy
The state_machine.c file inludes timer.h, state_machine.h, buzzer.h, led.h, switches.h
It is inluded in timer.c, and project_main.c
the state_machine.c includes the state updating method for switches
*/
#include <msp430.h>
#include "timer.h"
...
void state_update(){
  ...
  case START:
    ...
    break;
  //case dealing with when flashing green 
  case MENU_0:
    ...
    break;
  //state dealing with when flashing red
  case MENU_1:
    // Flash red
    setLed(1, 0);
